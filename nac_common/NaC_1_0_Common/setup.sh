...
if [[ "$1" != "sourceonly" ]]; then
    ...
    # install latest dnacentersdk
    #python -m pip install --upgrade dnacentersdk
    # install specific dnacentersdk
    python -m pip uninstall dnacentersdk -y
    python -m pip install dnacentersdk==2.9.1 
    # install latest ansible collection
    #ansible-galaxy collection install cisco.dnac --force --ignore-certs
    # install specific ansible collection
    #ansible-galaxy collection install cisco.dnac:6.33.2 --force --ignore-certs
    ansible-galaxy collection install cisco.dnac:6.31.3 --force --ignore-certs
fi
